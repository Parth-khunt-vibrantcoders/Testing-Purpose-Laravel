...
$('body').on('click', '.change-language', function() {
    ...
    console.log(value);
    if (value == 'EN') {
        var image = '226-united-states.svg';
    } else {
        if (value == 'SP') {
            var image = '128-spain.svg';
        } else {
            if (value == 'AR') {
                var image = '008-saudi-arabia.svg';
            } else {
                var image = '010-india.svg';
            }
        }
    }
    var html = '<img class="h-20px w-20px rounded-sm" src="' + baseurl + 'public/backend/media/svg/flags/' + image + '" alt="" />';
    $("#language-div").html(html);
...
$('body').on('click', '.notification-full-list', function(){
    $('#loader').show();
    $("#kt_body").attr('data-offcanvas-offcanvas', 'on');
    ...
    $.ajax({
        ...
        url: baseurl + "admin/common-ajaxcall",
        data: { 'action': 'get-notification-all-list'},
        success: function(data) {
            var output = JSON.parse(data);
            ...
            for(var i = 0; i < output.length; i++ ){
                ...
                if(output[i]['is_read'] == 'N'){
                    notification_class = 'not-read-notification';
                }else{
                    notification_class = 'read-notification';
                }

                html = html + ' <a href="'+ baseurl + output[i]['route'] + "/" +output[i]['slug'] +'" class="px-3 navi-item '+ notification_class +'">'+
                                    '<div class="navi-link">'+
                                        '<div class="navi-icon mr-2">';

                if(output[i]['module'] == 'Group Request'){
                    html  = html +  '<i class="icon-2x flaticon-users-1 flaticon-icon"></i>';
                }else{
                    if(output[i]['module'] == 'Group Admin Request'){
                        html  = html +  '<i class="icon-2x flaticon-user-settings flaticon-icon"></i>';
                    }else{
                        html  = html +  '<i class="icon-2x flaticon-star flaticon-icon"></i>';
                    }
                }
                html  = html +  '</div>'+
                                    '<div class="navi-text">'+
                                        '<div class="text font-weight-bold">'+ output[i]['title'] +'</div>'+
                                        '<div class="text">'+ output[i]['description'] +'</div>'+
                                    '</div>'+
                                '</div>'+
                            '</a>';
            }
            $("#notifications-list-all").html(html);
            // var html
        },
        complete: function(){
            $('#loader').hide();
...
$('body').on('click', '#kt_quick_panel_close', function(){
...
$('body').on('click', '.notification-btn', function(){
    ...
    $.ajax({
        ...
        url: baseurl + "admin/common-ajaxcall",
        data: { 'action': 'get-notification-list'},
        success: function(data) {
            var output = JSON.parse(data);
            // console.log(output.length);
            var html = '<div class="tab-pane active show " id="topbar_notifications_events" role="tabpanel">';

            if(output.length == 0){
                html = html + '<div class="scroll pr-7 mr-n7 text-center" data-scroll="true" data-height="30" data-mobile-height="30" style="margin: 35px 0">';
            }else{
                html = html + '<div class="navi navi-hover scroll my-4" data-scroll="true" data-height="300" data-mobile-height="200">';
            }
            ...
            if(output.length > 0){
                for(var i = 0; i < output.length; i++ ){
                    ...
                    html  = html + '<a href="'+ baseurl + output[i]['route'] + "/" +output[i]['slug'] +'" class="navi-item notification-list '+ notification_class +'">'+
                        '<div class="navi-link" style="padding: 0.5rem !important">'+
                            '<div class="navi-icon mr-2">';
                    if(output[i]['module'] == 'Group Request'){
                        html  = html +  '<i class="icon-2x flaticon-users-1 flaticon-icon"></i>';
                    }else{
                ...
                html =  html + "<p>You don't have any notification yet</p>";
            }
            // See All
            if(output.length > 0){
                html = html + '<div class="d-flex flex-center pt-7">'+
                                '<a href="javascript:;" class="btn btn-light-primary font-weight-bold text-center notification-full-list" id="kt_quick_panel_toggle">See All</a>'+
                            '</div>';
            }
            // See All End
            ...
            $("#notification_list_div").html(html);
